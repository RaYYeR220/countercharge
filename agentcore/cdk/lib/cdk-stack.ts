...
import { CountercodeFoundation } from './foundation';
...
export class AgentCoreStack extends Stack {
  ...
  /**
   * Countercharge foundation resources (Cognito, DynamoDB, S3, KMS, Secrets
   * Manager, the temporary probe Lambda). Tool targets and the agent service add their own constructs
   * (real Lambda tool targets, the Fargate agent service, ECR, ALB,
   * CloudFront, EventBridge Scheduler) to this same stack, referencing
   * `this.foundation.table` / `.bucket` / `.signingKey` / `.userPool` /
   * `.webClient` / `.systemClient` instead of re-declaring them.
   */
  public readonly foundation: CountercodeFoundation;
  ...
  constructor(scope: Construct, id: string, props: AgentCoreStackProps) {
    ...
    // Countercharge foundation: Cognito / DynamoDB / S3 / KMS / Secrets Manager.
    // Created first so downstream constructs (this stack's own AgentCoreMcp,
    // and later additions) can reference its outputs.
    this.foundation = new CountercodeFoundation(this, 'Foundation');
